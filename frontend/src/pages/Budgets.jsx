...
const getSummaryIcon = (status, pct) => {
  ...
  if (isOver) return (
    <div className="w-14 h-14 rounded-xl bg-red-100 flex items-center justify-center dark:bg-red-950/40">
  ...
  return (
    <div className="w-14 h-14 rounded-xl bg-green-100 flex items-center justify-center dark:bg-green-900/40">
    ...
    <Layout username={user}>
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {/* Month Navigation */}
          <div className="flex items-center justify-between">
            <button onClick={prevMonth} className="p-2 rounded-lg bg-white shadow-sm hover:bg-gray-50 border border-gray-200 transition-colors dark:bg-slate-800 dark:hover:bg-slate-700 dark:border-slate-700" aria-label="Poprzedni miesiac">
            ...
            <button onClick={nextMonth} className="p-2 rounded-lg bg-white shadow-sm hover:bg-gray-50 border border-gray-200 transition-colors dark:bg-slate-800 dark:hover:bg-slate-700 dark:border-slate-700" aria-label="Nastepny miesiac">
            ...
            {[
              ...
              <div key="spent" className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 dark:bg-slate-800 dark:border-slate-700">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {overallPct >= 80 ? (
                    <span className="px-2 py-0.5 rounded-full bg-red-100 text-red-600 text-xs font-semibold dark:bg-red-950/40 dark:text-red-400">Uwaga</span>
                  ) : (
              ...
              <div key="remaining" className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 dark:bg-slate-800 dark:border-slate-700">
          ...
          {/* Overall Progress */}
          {totalBudget > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 dark:bg-slate-800 dark:border-slate-700">
              <div className="flex items-center justify-between mb-3">
          ...
          {budgets.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {budgets.map((bud) => {
                ...
                return (
                  <div      key={'cat_' + bud.cat_id} className={`bg-white rounded-xl shadow-sm border transition-all hover:shadow-md dark:bg-slate-800 ${remainder < 0 ? 'border-red-200 dark:border-red-800/60' : 'border-gray-100 dark:border-slate-700'}`}>
                    {/* Header */}
                    <div className="flex items-center gap-3 px-5 py-4 pb-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-800 truncate dark:text-slate-200">{bud.category_name}</p>
                    ...
                    <div className="px-5 py-4 pt-3">
                      {limit > 0 ? (
                        <>
                          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden dark:bg-slate-700">
                        ...
                        <div>
                          <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden dark:bg-slate-700" />
                          <p className="text-xs text-gray-400 mt-2 dark:text-slate-500">Brak wydatków</p>
                        </div>
                      )}
                      ...
                      {limit > 0 ? (
                        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50 dark:border-slate-700">
                          <span className="text-xs text-gray-400 font-medium whitespace-nowrap dark:text-slate-500">Budżet:</span>
                           <input
                              type="text"
                              inputMode="numeric"
                              pattern="[0-9]*"
                             ...
                             onKeyDown={(e) => { if (e.key === 'Enter') handleSaveInline(bud.id, 'cat_' + bud.cat_id); }}
                                                       className="w-28 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-sm font-semibold text-gray-700 outline-none focus:border-[#32a852] focus:ring-1 focus:ring-green-100 transition-all tabular-nums dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                                                       />
                                                      <span className="text-xs text-gray-400 dark:text-slate-500">PLN</span>
                               {editingBudgets['cat_' + bud.cat_id] !== originalValues['cat_' + bud.cat_id] && (
                                        <>
                                          ...
                                          <button onClick={() => setEditingBudgets(prev => ({ ...prev, ['cat_' + bud.cat_id]: originalValues['cat_' + bud.cat_id] }))} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-gray-200 hover:bg-gray-300 text-gray-600 transition-colors dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300">
                                            Anuluj
                                          </button>
            </>
          )}
