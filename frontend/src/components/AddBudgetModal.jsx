    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="relative z-10 w-full max-w-md mx-4 rounded-xl bg-white shadow-2xl border border-gray-100 overflow-hidden dark:bg-slate-800 dark:border-slate-700">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {/* category pick */}
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1.5 dark:text-slate-300">Kategoria</label>
            {effectiveCats.length === 0 ? (
              catsLoading ? (
                <div className="flex items-center justify-center py-8">
                  <div className="w-5 h-5 border-2 border-gray-300 border-t-green-600 rounded-full animate-spin dark:border-slate-600 dark:border-t-[#32a852]" />
                </div>
              ) : (
                <p className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 dark:bg-amber-950/40 dark:border-amber-800/60 dark:text-amber-400">Brak kategorii — najpierw dodaj kategorię na stronie Kategorie.</p>
              )
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-2 bg-white dark:border-slate-600 dark:bg-slate-700">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                      selectedCatId === cat.id?.toString()
                        ...
                        : 'bg-gray-50 text-gray-700 hover:bg-gray-100 border border-transparent dark:bg-slate-600 dark:text-slate-200 dark:hover:bg-slate-500'
                    }`}
                  >
